...
interface AgentCardProps {
  item: AgentItem;
  onOpen: (item: AgentItem) => void;
  index?: number;
  compact?: boolean;
}

export function AgentCard({ item, onOpen, index = 0, compact }: AgentCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="absolute top-3 right-3 flex items-center gap-1 z-10">
        ...
        {item.isPopular && !item.isNew && (
          <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded-full bg-gradient-to-br from-purple-500 to-pink-500 text-white shadow-sm">
            HOT
          </span>
        )}
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <h3 className="font-semibold text-[15px] text-primary leading-tight truncate flex items-center gap-1">
              {item.name}
              {item.github && (
                <Github size={11} className="text-tertiary shrink-0" />
              )}
            </h3>
            <p className="text-[11px] text-tertiary mt-0.5 truncate">{item.developer}</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-1.5 pt-2 border-t divider">
        <button
          ...
          className="no-drag flex-1 inline-flex items-center justify-center gap-1 text-[11.5px] font-medium px-2 py-1.5 rounded-lg bg-gradient-to-b from-purple-500 to-pink-500 text-white shadow-sm hover:shadow-md transition-all hover:scale-[1.02] active:scale-[0.98]"
        >
